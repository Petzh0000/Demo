...
import { ShoppingCart, Star, Search, Loader2, CheckCircle } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-display font-medium text-sm transition-all ${
                    activeGroup === g.id
                      ...
                      : 'bg-white border border-gray-200 text-gray-600 hover:border-herb-green-400 hover:text-herb-green-700'
                  }`}
                >
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <p className="font-display text-gray-300 text-sm">ลองเปลี่ยน keyword หรือหมวดหมู่</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filtered.map(product => {
                ...
                return (
                  <div
                    key={product._id}
                    className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border border-gray-100 group"
                  >
                    {/* Image */}
                    ...
                    {/* Info */}
                    <div className="p-4">
                      <h3 className="font-display font-bold text-herb-dark text-sm leading-snug mb-1.5 line-clamp-2 min-h-[2.5rem]">
                        {product.name}
                      </h3>
                      <p className="text-gray-400 text-xs font-display mb-2 line-clamp-1">{product.description}</p>
                      <div className="flex items-center gap-0.5 mb-3">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className="w-3 h-3 text-herb-gold-500 fill-herb-gold-500" />
                        ))}
                        <span className="text-gray-400 text-xs font-display ml-1">(4.9)</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="font-display font-black text-herb-green-700 text-lg">
                        ...
                        >
                          {isAdded ? (
                            ...
                            <><ShoppingCart className="w-3 h-3" />เพิ่ม</>
                          ) : (
                            'หมด'
                          )}
                        </button>
